...
const groupBy = exports.groupBy = (rows, col, flags) => flags && flags.delimiter ? d3C.nest().key(row => row[`_${col}`]).object(rows.reduce((all, r) => r[col] ? all.concat(r[col].split(flags.delimiter).map(val => Object.assign({}, r, { [`_${col}`]: val }))) : all, [])) : d3C.nest().key(row => row[col]).object(rows);
